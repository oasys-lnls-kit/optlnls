...
def append_dataset_hdf5(
    ...
    """
    Append a caustic analysis dataset to an HDF5 file.

    This function stores a 2D beam intensity histogram and associated
    statistical beam properties into the ``datasets`` group of an HDF5 file.
    Each dataset corresponds to a propagation step and is saved under the
    name ``step_XXXXX``, where the number formatting is controlled by
    ``ndigits``.

    The function computes weighted beam statistics from the provided
    histogram projections and stores them as dataset attributes, together
    with mesh limits, beam dimensions, and optional Shadow-derived FWHM
    quantities.
    ...
    data : dict
        Dictionary containing histogram and beam profile information.

        This dictionary is typically generated by ``Shadow.Beam.histo2()``,
        although any mapping containing the required fields is accepted.

        Expected keys include:

        - ``histogram`` : 2D intensity histogram.
        - ``histogram_h`` : Horizontal projection histogram.
        - ``histogram_v`` : Vertical projection histogram.
        - ``bin_h_center`` : Horizontal bin centers.
        - ``bin_v_center`` : Vertical bin centers.
        - ``nbins_h`` : Number of horizontal bins.
        - ``nbins_v`` : Number of vertical bins.

        Optional Shadow-derived quantities:
        - ``fwhm_h``
        - ``fwhm_v``
        - ``fwhm_coordinates_h``
        - ``fwhm_coordinates_v``
    ...
    z : float
        Propagation position associated with the dataset.

    nz : int
        Total number of propagation steps.

    tag : int
        Dataset index identifier.

    t0 : float
        Initial reference time returned by ``time.time()``.

    ndigits : int
        Number of digits used in dataset name formatting.

    inmost_outmost : int, optional
        Mode passed to ``get_fwhm()`` controlling how the FWHM interval is
        determined.

    Notes
    -----
    The dataset is stored compressed using gzip compression.

    The following quantities are computed and stored as HDF5 attributes:
    ...
    If Shadow-derived FWHM values are unavailable, NaN values are stored
    and a warning message is printed.

    When the last propagation step is written (``tag == nz - 1``), the HDF5
    file attribute ``end time`` is updated with the current timestamp.
    """
